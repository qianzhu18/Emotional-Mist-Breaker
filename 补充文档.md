...
| tts.text.too\_long | 文本超过 10000 字符限制 |
| tts.voice\_id.not\_set | 用户未设置语音 |
...
| tool\_call | 工具调用开始，启用 WebSearch 时触发 |
| tool\_result | 工具调用结果，包含搜索查询和结果数量 |
| data | 聊天内容增量 |
| \[DONE\] | 流结束标志 |
...
| secondme.user.invalid\_id | 无效的用户 ID |
...
| secondme.act.action\_control.empty | actionControl 为空 |
| secondme.act.action\_control.too\_short | actionControl 过短（最少 20 字符） |
| secondme.act.action\_control.too\_long | actionControl 过长（最多 8000 字符） |
| secondme.act.action\_control.invalid\_format | 缺少 JSON 结构示例 |
...
](https://develop-docs.second.me/zh/docs/errors)
